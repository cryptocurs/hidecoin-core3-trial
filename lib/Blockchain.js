...
class Blockchain extends Component {
                                    ...
                                    .then((spends) => {
                                      Asyncs.forEach(spends, ({txHash, outN, spentAt}, i, next) => {
                                        this.restoreBlockIdFromTxHashSpentAt(txHash, spentAt, (blockId) => {
                                          this.db.query("UPDATE outs SET spentAt=? WHERE blockId=? AND txHash=? AND outN=?", [spentAt, blockId, txHash, outN]).then(() => next())
            ...
            Asyncs.forEach(blockData.txList, (tx, i, nextTx) => {
                ...
                .then(() => {
                  txData.txIns.eachAsync((txIn, i, raw, next) => {
                    ...
                    txData.txOuts.eachAsync(({address, value}, outN, raw, next) => {
                      this.db.query("INSERT INTO outs (blockId, blockHeight, txHash, outN, address, amount, inMasterBranch, spentAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [blockId, height, txHash, outN, address, value, isBranchMaster ? 1 : 0, 0]).then(next)
                    }, () => {
                      nextTx()
                    })
                  })
              ...
              if (isBranchMaster && queryUpdateOutsValues.length) {
                ...
                Asyncs.forEach(queryUpdateOutsValuesItems, (queryUpdateOutsValuesItem, i, next) => {
                  this.db.query("UPDATE outs SET spentAt=? WHERE (" + R.join(' OR ', R.repeat("txHash=? AND outN=?", queryUpdateOutsValuesItem.length >> 1)) + ") AND inMasterBranch=?", [blockId, ...queryUpdateOutsValuesItem, 1]).then(next)
                }, () => {
                  resolve()
                })
              } else {
  ...
  getCountByTime(since, till, callback, allowableLockCount = 0) {
    this.whenUnlocked((unlock) => {
      this.db.each("SELECT COUNT(id) cnt FROM blocks WHERE time>=? AND time<=?", [since, till], ({cnt}) => {
        unlock()
        callback(cnt)
      })
    }, allowableLockCount, 'Blockchain.getCountByTime()')
  }
