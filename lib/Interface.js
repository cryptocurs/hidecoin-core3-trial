  ...
  openWindow(name) {
    ...
    if (name === 'loading') {
      this.windows.loading = {
        logo: blessed.box({
          ...
          content: ('                                        \n' +
                    ...
                    '{center}{white-fg}Core III Version ' + storage.session.version + '{/white-fg}{/center}',
          tags: true,
          style: {
            fg: 'green',
            bg: 'blue',
      ...
      if (!this.windowsVars.app) {
        this.windowsVars.app = {headerType: 0, minerStates: ['/', '-', '\\', '|'], minerState: 0, progressMinerState: false, aliases: {}}
      }
          ...
          if (this.windowsVars.app.headerType === 1) {
            ...
            storage.emit('getLockQueueLength', (lockQueueLength) => {
              this.windows.app.header.setLine(0, '{bold}HPS ' + _.padStart(storage.session.stat.hps, 4)
                ...
                + ' MNR ' + this.windowsVars.app.minerStates[this.windowsVars.app.minerState] + _.padStart(storage.session.version, 9) + '{/bold}')
      ...
      if (name === 'loading') {
        ...
        if (data.currentBox) {
          ...
          if (data.currentBox === 'console') {
            ...
            const {content} = data
            this.windows.app.blocks.setContent(content)
            this.windows.app.blocks.setFront()
            this.windows.app.blocks.focus()
