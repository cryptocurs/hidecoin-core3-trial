...
const {Asyncs, Conv, Random, Time} = require('./helpers')
  ...
  run() {
    ...
    Time.synchronize((timeOffset) => {
      ...
      p2p.online(storage.config.net && storage.config.net.server && storage.config.net.server.port || 7438, () => {
        ...
        ifc.key('C-b', () => {
          ...
          blockchain.whenUnlocked((unlock) => {
            ...
            blockchain.getBranches((branches) => {
              Asyncs.forEach(branches, (branch, i, next) => {
                if (branch.isMaster) {
                  next()
                  return
                }
                ...
                blockchain.removeBranch(branch, next, 1)
              }, () => {
        ...
        if (process.argv.indexOf('--nosync') === -1) {
