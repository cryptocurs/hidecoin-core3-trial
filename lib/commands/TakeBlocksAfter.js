  ...
  addBlocks(callback) {
    ...
    blockchain.whenUnlocked((unlock) => {
      blockchain.getLength((blockchainLength) => {
        const each = this.fromAllBranches ? blockchain.eachAfter.bind(blockchain) : blockchain.eachInMasterBranchAfter.bind(blockchain)
        
        let added = 0
        each(this.afterHash, this.maxBlockCount, ({hash, data}, next) => {
