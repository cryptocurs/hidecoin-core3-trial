...
module.exports = class Block extends Component {
  ...
  isValidInBranchStructure(branchStructure, callback, allowableLockCount = 0) {
    blockchain.whenUnlocked((unlock) => {
      ...
      blockchain.getBlockByHash(this.data.prevBlock, (lastBlock, lastBlockHeight) => {
        ...
        const next = (count) => {
          ...
          Asyncs.forInc(1, this.data.txCount, (i, next) => {
            this.data.txList[i].isValidInBranchStructure(branchStructure, this.data, lastBlockHeight + 1, {isFirstBlockTx: false}, (valid, err, fee) => {
              if (valid) {
                notFirstBlockTxsFee += fee
                next()
              } else {
                ret(false, 'Wrong tx: ' + err)
              }
            }, allowableLockCount + 1)
          }, () => {
            this.data.txList[0].isValidInBranchStructure(branchStructure, this.data, lastBlockHeight + 1, {isFirstBlockTx: true, notFirstBlockTxsFee}, (valid, err) => {
              if (valid) {
                ret(true, null)
              } else {
                ret(false, 'Wrong tx: ' + err)
        ...
        if (lastBlockData) {
          blockchain.getCountByTimeInBranchStructure(branchStructure, lastBlockData.time - 3600, lastBlockData.time, (count) => {
            next(count)
          }, allowableLockCount + 1)
        } else {
          next()
    ...
    }, allowableLockCount, 'Block.isValidInBranchStructure()')
  }
}